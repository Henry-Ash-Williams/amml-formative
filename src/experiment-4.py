...
if __name__ == "__main__":
    print("Experiment 4:\n")
    print(
        "Investigates the performance of a Conditional VAE trained on a dataset with spatial transformations\n"
    )
    print("Training...\n")
    model, test_loss, train_loss = train(train_augment)
    torch.save(model, "E4-model.pt")
    pickle.dump(test_loss, open("E4-test-loss.pickle", "wb"))
    pickle.dump(train_loss, open("E4-train-loss.pickle", "wb"))
    print("Training complete\n")
    print(
        f"Final test loss on base test dataset: {test_loss['base'][-1]['loss'].item()}"
    )
    print(
        f"Classification Accuracy on base dataset: {get_linear_classification_acc(model, train_base, test_base, latent_size=NUM_CLASSES * LATENT_DIMS)}"
    )
    print(
        f"Final test loss on augmented test dataset: {test_loss['augmented'][-1]['loss'].item()}"
    )
    print(
        f"Classification Accuracy on augmented dataset: {get_linear_classification_acc(model, train_augment, test_augment, latent_size=NUM_CLASSES * LATENT_DIMS)}"
    )
