...
if __name__ == "__main__":
    print("Experiment 3:\n")
    print(
        "Investigates the performance of a Conditional VAE with a Spatial Transformer Network trained on a dataset without spatial transformations\n"
    )
    print("Training...\n")
    model, test_loss, train_loss = train(train_base)
    torch.save(model, "E3STN-model.pt")
    pickle.dump(test_loss, open("E3STN-test-loss.pickle", "wb"))
    pickle.dump(train_loss, open("E3STN-train-loss.pickle", "wb"))
